    ...
    def _do(self, problem, X, **kwargs):
        ...
        if float(self.CPBX) < np.random.random():
            ...
            for k in range(n_matings):
                ...
                until_which_block = np.random.randint(1, problem.NUMBER_POLYGONS)
                # print(f"until_which_block=",until_which_block)
                # print(f"problem.SIZE_BLOCK*until_which_block=",problem.SIZE_BLOCK*until_which_block)
                for i in range(0,problem.SIZE_BLOCK*until_which_block):
                    off_a[i] = b[i]
                    off_b[i] = a[i]
